"""
LangGraph Agent Implementation for SERTEC System
...
- Data Source: VisitReport (apps.visits.models)
"""
...
class SertecAgent:
    """
    Multi-step reasoning agent for the SERTEC System.
    
    Uses a simplified LangGraph-like pattern without the full library,
    to avoid heavy dependencies. Indexes and queries VisitReport exclusively.
    """
    ...
    def node_understand(self, state: AgentState) -> AgentState:
        ...
        quality_keywords = ['falla', 'defecto', 'soldadura', 'polifusión', 'electro', 'norma', 'procedimiento', 'ensayo', 'tubería', 'accesorio', 'calidad', 'garantía', 'reclamo', 'pasos', 'instalar', 'visita', 'reporte', 'muro', 'máquina']
        doc_keywords = ['documento', 'pdf', 'manual', 'buscar', 'guía', 'reporte', 'obra', 'cliente', 'historial', 'caso', 'problema', 'solución', 'informe', 'visita']
        ...
        # 5. Check for database queries (When was the last..., what visits...)
        ...
        # 6. Check for documents/visits (Local RAG)
        elif any(word in query_lower for word in doc_keywords):
            state['reasoning'] = "Se requiere búsqueda en el historial de documentos y reportes de visita."
            state['current_step'] = 'search_knowledge'
    ...
    def node_query_database(self, state: AgentState) -> AgentState:
        """Query the Django database for live VisitReport data"""
        ...
        try:
            ...
            stop_words = [
                'cuándo', 'cuando', 'fue', 'la', 'última', 'ultima', 'visita', 
            ...
            if keywords:
                for word in keywords:
                    query |= Q(report_number__icontains=word)
                    query |= Q(visit_reason__icontains=word)
                    query |= Q(client_name__icontains=word)
                    query |= Q(client_rut__icontains=word)
                    query |= Q(project_name__icontains=word)
                    query |= Q(address__icontains=word)
            
            # Get last 10 relevant visit reports
            reports = VisitReport.objects.filter(query).order_by('-created_at')[:10]
            
            db_results = []
            for rpt in reports:
                db_results.append({
                    'id': rpt.report_number,
                    'fecha': rpt.visit_date.strftime('%Y-%m-%d') if rpt.visit_date else 'N/A',
                    'cliente': rpt.client_name or rpt.client_rut or 'N/A',
                    'proyecto': rpt.project_name or 'N/A',
                    'estado': rpt.status,
                    'motivo': (rpt.visit_reason or '')[:200]
                })
            
            if db_results:
                msg = f"Se encontraron {len(db_results)} reportes de visita relevantes:\n"
                msg += "\n".join([f"- {r['id']} ({r['fecha']}): Cliente {r['cliente']}, Proyecto {r['proyecto']}, Estado {r['estado']}. Motivo: {r['motivo']}" for r in db_results])
                
                state['knowledge_results'] = state.get('knowledge_results', []) + [
                    {
                        'source': 'Base de Datos SERTEC (En Vivo)',
                        'content': msg
                    }
                ]
                state['reasoning'] += f" | {len(db_results)} reportes de visita encontrados en BD"
            else:
                state['reasoning'] += " | No se encontraron reportes de visita específicos en BD"
            ...
            if results and results.get('documents') and len(results['documents'][0]) > 0:
                ...
                for doc, meta in zip(docs, metas):
                    state['knowledge_results'].append({
                        ...
                        'visit_report_id': meta.get('visit_report_id')
                    })
    ...
    def node_generate_response(self, state: AgentState) -> AgentState:
        ...
        try:
            if self.provider_manager:
                prompt = f"""
Eres un Asistente Técnico Senior Multimodal del sistema SERTEC (Servicio Técnico en Terreno). 
Tu misión es asistir al personal técnico en diagnósticos, consultas de normativa y operaciones de visitas en terreno.

REGLAS DE ORO:
1. Si se te proporciona CONTEXTO RECUPERADO, úsalo como fuente de verdad prioritaria y cita fuentes (ej: RV-XXXX).
2. Si se te proporcionan ANÁLISIS DE IMAGEN, intégralos en tu respuesta técnica.
3. Si el usuario hace preguntas generales o de cortesía, responde de forma amable y profesional.
...
5. Tu contexto de datos proviene EXCLUSIVAMENTE de Reportes de Visita (VisitReport). NO existen Incidencias en este sistema.

Consulta del usuario: {state['query']}

CONTEXTO RECUPERADO DE SERTEC:
{chr(10).join(context_parts) if context_parts else 'No se encontró información histórica específica. Responde basándote en estándares técnicos generales de servicio en terreno.'}
...
def get_agent() -> SertecAgent:
    """Get the singleton agent instance"""
    global _agent_instance
    if _agent_instance is None:
        _agent_instance = SertecAgent()
