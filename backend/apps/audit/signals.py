...
def get_current_user(instance):
    ...
    # 1. Priorizar el usuario del contexto actual (Middleware/ThreadLocal)
    from apps.core.thread_local import get_current_user as get_thread_user
    thread_user = get_thread_user()
    if thread_user and thread_user.is_authenticated:
        return thread_user
        
    # 2. Fallback a campos del modelo
...
def audit_strict_changes(sender, instance, created, **kwargs):
    ...
    ALLOWED_MODELS = ['incident', 'report', 'user', 'clientqualityreport', 'visitreport']
    ...
    if created:
        ...
        elif model_name == 'report' or model_name == 'visitreport':
            action_code = 'report_created' 
            desc = f"Creó reporte de visita: {instance}"
    ...
    # 2. ACTUALIZACIONES ESPECÍFICAS (Escalar, Cerrar)
        ...
        # Reportes de visita: Auditar cambios generales
        if model_name == 'visitreport':
            AuditService.log_action(
                user=user,
                action='report_updated', # "Actualizar"
                description=f"Actualizó reporte de visita: {instance}",
                details={'model': model_name, 'pk': instance.pk}
            )
...
def audit_delete_and_recycle(sender, instance, **kwargs):
    ...
    ALLOWED_RECYCLE_MODELS = ['incident', 'report', 'user', 'clientqualityreport', 'visitreport']
