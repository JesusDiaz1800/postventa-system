...
ALLOWED_HOSTS = os.getenv('DJANGO_ALLOWED_HOSTS', '127.0.0.1,localhost,testserver,sertec.polifusion.com,api-sertec.polifusion.com,*').split(',')

# --- Security & Proxy (Forced for Tunnel) ---
...
MIDDLEWARE = [
    ...
    # CsrfViewMiddleware removido: la API usa JWT (Bearer tokens), no cookies de sesión.
    # CSRF solo es necesario para autenticación basada en cookies/sesiones.
...
def _build_database_config_from_env():
    ...
    if database_url:
        ...
        name = parsed.path.lstrip('/') if parsed.path else os.getenv('DB_NAME', 'SERTEC_CL')
    ...
    return {
        'ENGINE': 'mssql',
        'NAME': os.getenv('DB_NAME', 'SERTEC_CL'),
...
DATABASES = {
    ...
    'default_pe': {
        'ENGINE': 'mssql',
        'NAME': os.getenv('DB_NAME_PE', 'SERTEC_PE'),
        'USER': os.getenv('DB_USER_PE', 'ccalidad'),
        'PASSWORD': os.getenv('DB_PASSWORD_PE', os.getenv('DB_PASSWORD', 'Plf2025**')),
        'HOST': os.getenv('DB_HOST', 'localhost\\SQLEXPRESS'),
        'PORT': os.getenv('DB_PORT', ''),
        'OPTIONS': {
            'driver': os.getenv('ODBC_DRIVER', 'ODBC Driver 18 for SQL Server'),
    ...
    'default_co': {
        'ENGINE': 'mssql',
        'NAME': os.getenv('DB_NAME_CO', 'SERTEC_CO'),
        'USER': os.getenv('DB_USER_CO', 'ccalidad'),
        'PASSWORD': os.getenv('DB_PASSWORD_CO', os.getenv('DB_PASSWORD', 'Js2024**')),
        'HOST': os.getenv('DB_HOST', 'localhost\\SQLEXPRESS'),
        'PORT': os.getenv('DB_PORT', ''),
        'OPTIONS': {
            'driver': os.getenv('ODBC_DRIVER', 'ODBC Driver 18 for SQL Server'),
...
# DATABASES['default_pe']['NAME'] = 'SERTEC_PE'
# DATABASES['default_co']['NAME'] = 'SERTEC_CO' 
...
# Usar Memoria Interna para todo (Evita dependencia de Redis en Windows/Portable)
CACHES = {
    'default': {
        'BACKEND': 'django.core.cache.backends.locmem.LocMemCache',
        'LOCATION': 'unique-snowflake',
...
# Solo incluir directorios que existen para evitar staticfiles.W004
_static_candidates = [
    str(os.path.join(BASE_DIR, 'static')),
    str(FRONTEND_DIST_DIR),
]
STATICFILES_DIRS = [d for d in _static_candidates if os.path.isdir(d)]
...
CORS_ALLOW_ALL_ORIGINS = True  # Dev/LAN - set to False in production with strict whitelist
...
CORS_ALLOWED_ORIGINS = [
    "http://localhost:5173",
    "http://localhost:5174",
    "https://sertec.polifusion.com",
    "https://api-sertec.polifusion.com",
    "https://incidencias.polifusion.com",
]
...
CORS_ALLOW_CREDENTIALS = True
CORS_ALLOW_ALL_ORIGINS = True # Temporalmente más permisivo para depurar móvil

# Configuración de Cookies Blindada para Producción HTTPS
SESSION_COOKIE_DOMAIN = None
CSRF_COOKIE_DOMAIN = None
SESSION_COOKIE_SAMESITE = 'None'
CSRF_COOKIE_SAMESITE = 'None'
SESSION_COOKIE_SECURE = True # Requerido para HTTPS a través del Túnel
CSRF_COOKIE_SECURE = True
# --- Email Configuration (SMTP) ---
EMAIL_BACKEND = 'django.core.mail.backends.smtp.EmailBackend'
EMAIL_HOST = 'mail.polifusion.cl'
EMAIL_PORT = 587
EMAIL_USE_TLS = True
EMAIL_USE_SSL = False
EMAIL_HOST_USER = 'postventapolifusion@polifusion.cl'
# La contraseña debe configurarse en el archivo .env como EMAIL_HOST_PASSWORD
EMAIL_HOST_PASSWORD = os.getenv('EMAIL_HOST_PASSWORD', '')
DEFAULT_FROM_EMAIL = 'Sertec Polifusion <postventapolifusion@polifusion.cl>'
SERVER_EMAIL = EMAIL_HOST_USER

# --- SAP Service Layer Configuration ---
SAP_SL_BASE_URL = os.getenv('SAP_SL_BASE_URL', 'https://192.168.1.150:50000/b1s/v1')
SAP_SL_COMPANY_DB = os.getenv('SAP_SL_COMPANY_DB', 'SERTEC_CL')
SAP_SL_COMPANY_DB_PE = os.getenv('SAP_SL_COMPANY_DB_PE', 'SERTEC_PE')
SAP_SL_COMPANY_DB_CO = os.getenv('SAP_SL_COMPANY_DB_CO', 'SERTEC_CO')

# SAP Credentials (Mapped to ENV)
SAP_SL_USER_CL = os.getenv('SAP_SL_USER_CL', 'ccalidad')
SAP_SL_PASS_CL = os.getenv('SAP_SL_PASS_CL', 'Plf5647**')
SAP_SL_USER_PE = os.getenv('SAP_SL_USER_PE', 'jefsertec_ppe')
SAP_SL_PASS_PE = os.getenv('SAP_SL_PASS_PE', 'tec006')
SAP_SL_USER_CO = os.getenv('SAP_SL_USER_CO', 'jefsertec_pco')
SAP_SL_PASS_CO = os.getenv('SAP_SL_PASS_CO', 'Js2024**')

# --- WhatsApp / AI Configurations ---
