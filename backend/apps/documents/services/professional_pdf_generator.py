...
class ProfessionalPDFGenerator:
    ...
    def _draw_header_footer(self, canvas, doc):
        ...
        if self.logo_path:
            try:
                if os.path.exists(self.logo_path):
                    ...
                    logger.warning(f"Logo path exists in var but not in disk: {self.logo_path}")
            except Exception as logo_err:
                logger.error(f"Error drawing logo in PDF: {logo_err}")
                pass
        ...
        canvas.drawRightString(self.page_width - self.margin_x, self.margin_y - 10, f"Página {canvas.getPageNumber()}")
    ...
    def _get_section_images_flow(self, report_id, section):
        ...
        attachments = DocumentAttachment.objects.filter(
            document_type__in=['quality_report', 'visit_report'],
    ...
    def generate_visit_report_pdf(self, data: Dict, output_path_or_buffer) -> bool:
        ...
        try:
            ...
            sap_id = data.get('sap_doc_num') or data.get('sap_call_id') or 'N/A'
            elements.append(Paragraph(f"N° Reporte: {data.get('report_number', '-')} | Ref. SAP: {sap_id} | Fecha: {visit_date}", self.styles['ModernSubtitle']))
            ...
            general_info = {
                ...
                "Contacto y Tel:": f"{data.get('client_contact', '-')} | {data.get('telephone') or data.get('client_phone') or '-'}"
            }
            elements.append(self._create_info_block(general_info))
            
            # 2.1 Información de la Incidencia y Producto (NUEVO)
            if data.get('incident_code') or data.get('product_category'):
                elements.append(Paragraph("DETALLES DE LA INCIDENCIA Y PRODUCTO", self.styles['SectionHeader']))
                incident_info = {
                    "Incidencia:": data.get('incident_code'),
                    "Prioridad:": data.get('incident_priority'),
                    "Responsable:": data.get('incident_responsible'),
                    "Categoría:": data.get('product_category'),
                    "Subcategoría:": data.get('product_subcategory'),
                    "Proveedor:": data.get('product_provider'),
                    "Fecha Det.:": data.get('incident_detection_date'),
                    "Hora Det.:": data.get('incident_detection_time'),
                }
                elements.append(self._create_info_block(incident_info))
                
                if data.get('incident_description'):
                    elements.append(Paragraph("<b>Descripción:</b>", self.styles['FieldLabel']))
                    elements.append(Paragraph(data.get('incident_description'), self.styles['ModernBodyText']))
                    elements.append(Spacer(1, 3))
                
                if data.get('incident_immediate_actions'):
                    elements.append(Paragraph("<b>Acciones Inmediatas:</b>", self.styles['FieldLabel']))
                    elements.append(Paragraph(data.get('incident_immediate_actions'), self.styles['ModernBodyText']))
                    elements.append(Spacer(1, 3))
            ...
            personnel_info = {
                ...
                "Telf. Inst.:": data.get('installer_phone', '-') # Antes Prof. Obra, ahora el dato del contacto del proyecto
            }
            elements.append(self._create_info_block(personnel_info))

            
            # --- NUEVA UBICACIÓN: MÉTRICAS E INFORMACIÓN DE OBRA ---
            ...
            # Mapeo de Nivel de Instalación (ID -> Texto Legible para el Cliente)
            level_map = {
                '0': 'NORMAL',
                '1': 'DEFICIENTE',
                '2': 'CRÍTICO',
            ...
            raw_level = str(data.get('installation_level', '0'))
            # Si ya es texto (compatible con lo anterior), lo usamos, si no, lo mapeamos
            inst_level_text = level_map.get(raw_level, raw_level.upper())

            complementary_info = {
                "Obra Finalizada:": bool_to_str(data.get('is_project_finished')),
                "Material Mezclado:": bool_to_str(data.get('is_mixed_material')),
                "Obra Rescatada/Nueva:": bool_to_str(data.get('is_rescued_project')),
                "Obra con otro Prov.:": bool_to_str(data.get('is_other_provider')),
                "Nivel Instalación:": inst_level_text,
                "Nombre Proveedor:": data.get('other_provider_name', '-'),
            }
            elements.append(self._create_info_block(complementary_info))

            # Extraer y sanitizar machine_data (puede venir como string o dict dependiendo del JSONField)
            raw_machine_data = data.get('machine_data', {})
            if isinstance(raw_machine_data, str):
                try:
                    import json
                    raw_machine_data = json.loads(raw_machine_data)
                except:
                    raw_machine_data = {}
            if not isinstance(raw_machine_data, dict):
                raw_machine_data = {}

            machines = raw_machine_data.get('machines', [])
            if not isinstance(machines, list):
                machines = []
            
            # Filtrar máquinas vacías
            machines = [m for m in machines if m.get('machine') or m.get('modelo')]
            ...
            if machines:
                ...
                if raw_machine_data.get('machine_removal'):
                    elements.append(Spacer(1, 3))
                    elements.append(Paragraph(f"<b>Retiro de Máquina:</b> SÍ (N° Reporte de Retiro: {raw_machine_data.get('report_number', '-')})", self.styles['ModernBodyText']))
            ...
            # Sanitizar visit_reason
            visit_reason_val = data.get('visit_reason_display') or "Visita Técnica"
            elements.append(Paragraph(f"<b>Motivo Visita:</b> {visit_reason_val}", self.styles['ModernBodyText']))
            ...
            if materials:
                ...
                # headers
                table_data = [['Material / Producto', 'Cant.', 'Marca', 'Lote']]
                for m in materials:
                    # Adaptarse a la nueva estructura de materiales_data
                    name = m.get('material') or m.get('material_name') or m.get('name') or '-'
                    qty = m.get('cantidad') or m.get('quantity', '-')
                    brand = m.get('marca') or '-'
                    lot = m.get('lote') or m.get('batch') or '-'
                    table_data.append([name, qty, brand, lot])
            ...
            # 6. Registro Fotográfico (Imágenes de terreno)
            # Escanear 'attachments' o 'images' (inyectado desde la vista) en data
            images_to_show = []
            
            # Prioridad 1: Imágenes inyectadas directamente (desde el serializer o regeneración)
            raw_images = data.get('images', [])
            if raw_images:
                images_to_show.extend(raw_images)
            
            # Prioridad 2: Escanear 'attachments' serializados
            ...
            if attachments:
                for att in attachments:
                    ...
                    # Evitar duplicados si ya están en images_to_show
                    if any(img.get('path') and att.get('file') in img.get('path') for img in images_to_show):
                        continue
                        
                    # Filtrar por extensiones de imagen
                    if any(file_url.lower().endswith(ext) for ext in ['.jpg', '.jpeg', '.png']):
                        # Obtener el path absoluto
                        try:
                            # Intentar obtener path desde el objeto si está disponible (vía backend)
                            # Si es URL, procesar
                            from urllib.parse import urlparse
                            parsed_url = urlparse(file_url)
                            rel_path = parsed_url.path
                            
                            # Limpiar prefijos de URL/Media
                            for prefix in ['/documentos/', 'documentos/', '/media/', 'media/']:
    ...
    def _build_signatures_block(self, data):
        ...
        client_sign_img = None
        client_signature_base64 = data.get('client_signature')
        
        if client_signature_base64:
            try:
                import base64
                from reportlab.lib.utils import ImageReader
                
                # Limpiar el prefijo data:image/png;base64, si existe
                if ';base64,' in client_signature_base64:
                    client_signature_base64 = client_signature_base64.split(';base64,')[1]
                
                img_data = base64.b64decode(client_signature_base64)
                img_stream = io.BytesIO(img_data)
                
                # Crear imagen para ReportLab
                client_sign_img = ReportLabImage(img_stream, width=4*cm, height=2*cm)
                client_sign_img.hAlign = 'CENTER'
            except Exception as sign_error:
                logger.warning(f"Error procesando firma del cliente: {sign_error}")
        ...
        content_client = []
        if client_sign_img:
            content_client.append(client_sign_img)
            content_client.append(Spacer(1, 3))
        else:
            content_client.append(Spacer(1, client_spacer_height))
            
        content_client.append(Paragraph(client_signature_name, style_name))
        content_client.append(Spacer(1, 2))
        content_client.append(Paragraph(role_2_title, style_label))
