...
def get_direct_file_url(document_type, report_id, filename):
    ...
    direct_url = f"{documents_url}{folder_name}/report_{report_id}/{filename}"
    return direct_url

from ..models import DocumentTemplate, Document, DocumentVersion, DocumentConversion, VisitReport
from ..serializers import (
    ...
    VisitReportSerializer, VisitReportCreateSerializer, 
...
@permission_classes([permissions.IsAuthenticated])
def documents_by_visits(request):
    """Get documents organized by visit reports (REPLACES documents_by_incidents)"""
    logger.info(f"Documents by visits called by user: {request.user.username}")
    
    try:
        from rest_framework.pagination import PageNumberPagination
        
        # Base VisitReport Queryset
        queryset = VisitReport.objects.all().prefetch_related('documents').order_by('-created_at')
        ...
        visits_list = []
        for visit in (page if page is not None else queryset):
            visit_docs = []
            
            # 1. Documents directly linked to this visit
            ...
            visits_list.append({
                ...
                'documents': visit_docs
            })
        
        if page is not None:
            return paginator.get_paginated_response(visits_list)
        ...
        logger.error(f"Error in documents_by_visits: {e}")
...
@api_view(['POST'])
@permission_classes([permissions.IsAuthenticated])
def generate_document(request, visit_id=None):
    """Generate document from template (REPLACES incident_id with visit_id)"""
    ...
    visit = None
    if visit_id:
        visit = get_object_or_404(VisitReport, id=visit_id)
...
# Eliminados: generate_polifusion_lab_report, generate_polifusion_incident_report
