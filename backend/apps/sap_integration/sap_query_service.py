...
class SAPQueryService:
    ...
    def get_technicians(self, only_technical_role=False):
        """
        Obtener lista de técnicos/empleados activos vía SQL.
        Validación: OHEM.Active = 'Y' y HEM6.roleID = 2
        Si no se encuentran técnicos con ese rol (ej. base de datos de prueba),
        se cae en fallback a todos los empleados activos para evitar desplegables vacíos.
        """
        cache_key = f'sap_technicians_{get_current_country()}_v3'
        ...
        try:
            technicians = []
            with connections[self._get_db_alias()].cursor() as cursor:
                # 1. Consulta estricta según requerimiento (roleID = 2)
                query = """
                    SELECT DISTINCT T0.empID, (T0.firstName + ' ' + T0.lastName) as Name, T0.email 
                    FROM OHEM T0 
                    INNER JOIN HEM6 T1 ON T0.empID = T1.empID 
                    WHERE T0.Active = 'Y' AND T1.roleID = 2
                    ORDER BY Name
                """
                ...
                # 2. Fallback robusto para bases de datos de prueba sin roles configurados
                if not technicians:
                    logger.warning(f"SAP: No se encontraron técnicos con roleID = 2. Aplicando fallback de empleados activos.")
                    fallback_query = """
            ...
            logger.error(f"Error fetching technicians: {sql_err}")
            return []
    ...
    def get_customer_full_details(self, card_code):
        ...
        try:
            from django.db import connections
            with connections[self._get_db_alias()].cursor() as cursor:
                # 1. Datos básicos del cliente + vendedor + posibles UDFs de instalador en OCRD
                # Intentamos obtener UDFs comunes de Polifusión si existen
                try:
                    cursor.execute("""
                        SELECT 
                            c.CardCode, c.CardName, c.SlpCode, s.SlpName, s.Email as SalespersonEmail,
                            c.Address, c.Phone1, c.E_Mail,
                            c.U_nx_inst_nom, c.U_nx_fono_inst
                        FROM OCRD c
                        LEFT JOIN OSLP s ON c.SlpCode = s.SlpCode
                        WHERE c.CardCode = %s
                    """, [card_code])
                except Exception:
                    # Fallback si no existen los UDFs en OCRD
                    cursor.execute("""
                        SELECT 
                            c.CardCode, c.CardName, c.SlpCode, s.SlpName, s.Email as SalespersonEmail,
                            c.Address, c.Phone1, c.E_Mail
                ...
                customer_data = {
                    ...
                    'phone': row[6],
                    'email': row[7],
                    'installer': row[8] if len(row) > 8 else None,
                    'installer_phone': row[9] if len(row) > 9 else None,
                    'addresses': [],
                    'projects': []
                }
    ...
    def get_service_call(self, doc_num):
        ...
        try:
            ...
            customer_data = None
            if not self.order_number or self.order_number == self.report_number:
            ...
            # Obtener nombre del contacto desde OCPR y Teléfono de OSCL
            contact_name = None
            service_call_phone = None
            try:
                from django.db import connections
                with connections[self._get_db_alias()].cursor() as cursor:
                    # Traer nombre del contacto y el teléfono específico de la llamada (OSCL.Telephone)
                    cursor.execute("""
                        SELECT T0.Name, T1.Telephone 
                        FROM OCPR T0 
                        RIGHT JOIN OSCL T1 ON T0.CntctCode = T1.cntctCode
                        WHERE T1.docNum = %s
                    """, [doc_num])
                    row = cursor.fetchone()
                    if row:
                        contact_name = row[0]
                        service_call_phone = row[1]
            except Exception as e:
                logger.warning(f"Could not fetch contact/phone details for docNum {doc_num}: {e}")
            ...
            result = {
                ...
                'contact_person': contact_name or (customer_data.get('ContactPerson', '') if customer_data else ''),
                'status': call.status,
                ...
                'telephone': service_call_phone or (customer_data.get('phone', '') if customer_data else ''), # OSCL.Telephone prioritario
                'customer_phone': customer_data.get('phone', '') if customer_data else '', # Telefono OCRD (el que estaba en contacto antes)
    ...
    def get_service_call_by_id(self, call_id):
        """Obtener llamada de servicio por su ID interno (ServiceCallID)"""
        try:
            with connections[self._get_db_alias()].cursor() as cursor:
                cursor.execute("SELECT docNum FROM OSCL WHERE callID = %s", [call_id])
                row = cursor.fetchone()
                if row:
                    return self.get_service_call(row[0])
            return None
        except Exception as e:
            logger.error(f"Error fetching service call by ID {call_id}: {e}")
            return None
    ...
    def get_customer_projects(self, card_code):
        """Obtener obras de un cliente con todos los campos de @FRMOBRAS"""
        ...
        try:
            ...
            with connections[self._get_db_alias()].cursor() as cursor:
                query = """
                    SELECT 
                        Code, Name,
                        U_nx_direccion, U_nx_comuna, U_nx_ciudad,
                        U_nx_construc,
                        U_nx_inst_nom, U_nx_fono_inst,
                        U_prof_obra_nom, U_ito_nom
                    FROM [@FRMOBRAS]
                    WHERE U_nx_cardcode LIKE %s
                    ORDER BY Name
                """
                ...
                for row in rows:
                    projects.append({
                        ...
                        'comuna':               row[3] or '',
                        'ciudad':               row[4] or '',
                        'construction_company': row[5] or '',
                        'installer':            row[6] or '',
                        'installer_phone':      row[7] or '',
                        'professional_name':    row[8] or '',
                        'technical_inspector':  row[9] or '',
                    })
