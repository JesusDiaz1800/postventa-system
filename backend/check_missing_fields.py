#!/usr/bin/env python
"""
Script para verificar qué campos están faltando en la base de datos
"""
import os
import sys
import django
...
def main():
    print("=== VERIFICACION DE CAMPOS FALTANTES ===")
    ...
    try:
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='incidents_incident';")
                table_exists = cursor.fetchone()
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='incidents_incidenttimeline';")
                table_exists = cursor.fetchone()
            ...
            if table_exists:
                ...
                with connection.cursor() as cursor:
                    cursor.execute("PRAGMA table_info(incidents_incidenttimeline);")
                    columns = [row[1] for row in cursor.fetchall()]
