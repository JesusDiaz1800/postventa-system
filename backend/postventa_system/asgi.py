...
class QueryAuthMiddleware(BaseMiddleware):
    ...
    async def __call__(self, scope: Dict[str, Any], receive: Any, send: Any) -> None:
        try:
            ...
            if token:
                print(f"[ASGI] Intentando autenticar con token para país: {country_code}")
    ...
    def get_user_from_token(self, token: str, country_code: str):
        ...
        try:
            # 1. Configurar contexto de país inmediatamente
            from apps.core.thread_local import set_current_country
            set_current_country(country_code)
            
            # 2. Decodificar Token (Manual para evitar fallos de inicialización de SimpleJWT)
            import jwt
            from django.conf import settings
            
            try:
                # Intentamos decodificar con la SECRET_KEY de Django
                payload = jwt.decode(token, settings.SECRET_KEY, algorithms=['HS256'])
                user_id = payload.get('user_id')
            except Exception as jwt_err:
                print(f'[ASGI] Fallo decodificación manual JWT: {str(jwt_err)}')
                # Intento fallback con AccessToken de SimpleJWT
            ...
            if user_id:
                ...
                print(f"[ASGI] Usuario autenticado exitosamente: {user.username} ({country_code})")
                return user
                
            return None
        except Exception as e:
            print(f'[ASGI] Error CRÍTICO validando JWT token: {str(e)}')
            import traceback
            traceback.print_exc()
            return None
...
# Consumer que rechaza cualquier WebSocket no registrado (evita ValueError en logs)
class DenyConsumer:
    """Cierra limpiamente conexiones WebSocket a rutas no registradas."""
