...
USER = 'ccalidad'
# Try connection with known working password logic or hardcode valid one
# But for safety, let's use the one that worked in test_pe_connection.py
PASSWORD = 'Plf2025**' 
DATABASE = 'POSTVENTA_PE'
DRIVER = '{ODBC Driver 13 for SQL Server}'

def get_connection():
    # Attempt connection with PLf2025** which worked
...
try:
    ...
    if current_db.lower() != 'postventa_pe':
        print(f"[FATAL ERROR] Refusing to clean. Connected to '{current_db}' instead of 'POSTVENTA_PE'.")
        conn.close()
        exit(1)
