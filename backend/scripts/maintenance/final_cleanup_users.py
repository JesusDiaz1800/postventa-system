#!/usr/bin/env python
"""
Script final para limpiar usuarios y configurar usuario real
"""
import os
import sys
import django
...
def final_cleanup_users():
    """Limpieza final de usuarios y configuración del usuario real"""
    try:
        print("Limpiando usuarios de prueba...")
        
        with connection.cursor() as cursor:
            # Limpiar tabla auth_user
            test_users = ['admin', 'testuser', 'simpleuser', 'djangouser']
            for username in test_users:
                cursor.execute("DELETE FROM auth_user WHERE username = %s", [username])
                print(f"  - Usuario '{username}' eliminado de auth_user")
            
            # Limpiar tabla users
            test_users_custom = ['customuser', 'admin']
            for username in test_users_custom:
                cursor.execute("DELETE FROM users WHERE username = %s", [username])
                print(f"  - Usuario '{username}' eliminado de users")
