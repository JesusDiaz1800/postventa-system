#!/usr/bin/env python
"""
Script para crear incidencia completa con todos los campos NOT NULL
"""
import os
import sys
import django
...
def create_complete_incident():
    """Crear incidencia completa con todos los campos"""
    try:
        print("Creando incidencia completa...")
        
        with connection.cursor() as cursor:
            # Insertar incidencia con TODOS los campos NOT NULL
            cursor.execute("""
                INSERT INTO incidents (
                    ...
                    datetime('now'), date('now'), time('now'), 'Descripción de prueba', 'Calidad', 'Alta', 'abierto',
                    datetime('now')
                )
            """)
