#!/usr/bin/env python
"""
Script para probar la IA real y funcional
"""
import os
import sys
import django
...
from apps.ai.real_ai_service import RealAIService
import tempfile
import base64

def test_real_ai():
    """Probar la IA real con una imagen de prueba"""
    
    print("=== PROBANDO IA REAL Y FUNCIONAL ===")
    ...
    try:
        # Inicializar servicio de IA real
        ...
        for analysis_type in analysis_types:
            ...
            if result['success']:
                ...
                print(f"💰 Costo estimado: ${result.get('cost_estimate', 0):.6f}")
                print(f"🔢 Tokens usados: {result.get('tokens_used', 0)}")
                print(f"📝 Análisis (primeros 200 caracteres):")
        ...
        if report_result['success']:
            ...
            print(f"💰 Costo estimado: ${report_result.get('cost_estimate', 0):.6f}")
            print(f"📝 Reporte (primeros 200 caracteres):")
            print(f"   {report_result['report'][:200]}...")
        else:
            print(f"❌ Error en reporte: {report_result.get('error', 'Unknown error')}")
        
        print(f"\n🎉 Prueba de IA real completada!")
