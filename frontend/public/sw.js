const CACHE_NAME = 'sertec-v1.1';  // Forzar actualización total tras cambio de modal
const urlsToCache = [
    '/',
    '/index.html',
    '/manifest.json',
...
self.addEventListener('message', (event) => {
    ...
    // Responder SIEMPRE si se espera respuesta (evita el error de canal cerrado)
    if (event.ports && event.ports[0]) {
        try {
            event.ports[0].postMessage({ status: 'ok', received: true });
        } catch (e) {
            console.warn('[SW] Error al responder mensaje:', e);
        }
    }
});
