...
const ConfirmationModal = ({
    ...
    const getIcon = () => {
        switch (type) {
            case 'danger':
            case 'warning':
                return <ExclamationTriangleIcon className={`h-6 w-6 ${type === 'danger' ? 'text-red-600' : 'text-yellow-600'}`} aria-hidden="true" />;
            case 'info':
            default:
    ...
    const getButtonClass = () => {
        const base = "inline-flex w-full justify-center rounded-md px-3 py-2 text-sm font-semibold text-white shadow-sm sm:ml-3 sm:w-auto";
        switch (type) {
            case 'danger':
                return `${base} bg-red-600 hover:bg-red-500`;
            case 'warning':
                return `${base} bg-yellow-600 hover:bg-yellow-500`;
            case 'info':
            default:
