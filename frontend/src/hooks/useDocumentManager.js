...
export const useDocumentManager = () => {
  ...
  const getDocumentsByIncident = useCallback((incidentId, documentType) => {
    ...
    } else if (['supplier_report', 'quality_report', 'lab_report', 'visit'].includes(documentType)) {
      // Normalizar 'visit' a 'visit_report' si es necesario para el backend
      const backendType = documentType === 'visit' ? 'visit_report' : documentType;
      return api.get(`/documents/report-attachments/${incidentId}/${backendType}/`);
    }
  ...
  const uploadDocument = useMutation({
    mutationFn: async ({ file, incidentId, documentType, description = '', isPublic = false }) => {
      ...
      if (documentType === 'incident_attachments') {
        ...
        const backendType = documentType === 'visit' ? 'visit_report' : documentType;
        endpoint = `/documents/report-attachments/${incidentId}/${backendType}/upload/`;
      } else {
  ...
  const deleteDocument = useMutation({
    mutationFn: async ({ incidentId, documentType, attachmentId }) => {
      if (documentType === 'incident_attachments') {
        ...
        const backendType = documentType === 'visit' ? 'visit_report' : documentType;
        return api.delete(`/documents/report-attachments/${incidentId}/${backendType}/${attachmentId}/delete/`);
      }
  ...
  const downloadDocument = useCallback(async (incidentId, documentType, attachmentId) => {
    try {
      ...
      if (documentType === 'incident_attachments') {
        ...
        const backendType = documentType === 'visit' ? 'visit_report' : documentType;
        endpoint = `/documents/report-attachments/${incidentId}/${backendType}/${attachmentId}/download/`;
      } else {
  ...
  const openDocument = useCallback(async (incidentId, documentType, attachmentId) => {
    ...
    // Normalizar el tipo para las URLs de visualización
    let backendType = documentType;
    if (documentType === 'visit') backendType = 'visit_report';
    
    if (documentType === 'incident_attachments') {
      url = `${API_ORIGIN}/api/documents/incident-attachments/${incidentId}/${attachmentId}/view/`;
    } else if (['supplier_report', 'quality_report', 'lab_report', 'visit_report', 'visit'].includes(documentType)) {
      url = `${API_ORIGIN}/api/documents/report-attachments/${incidentId}/${backendType}/${attachmentId}/view/`;
    } else {
