...
const Documents = () => {
  ...
  // Fetch incidents with backend search
  const { data: incidentsData, isLoading, error, refetch } = useQuery({
    queryKey: ['documents-incidents', filters, debouncedSearchTerm],
    queryFn: async () => {
  ...
  // Filter incidents locally if needed (currently minimal as backend handles most)
  const filteredIncidents = useMemo(() => {
    return incidents; // Backend handles primary search and filters now
  }, [incidents]);

  const handleRefresh = () => {
    refetch();
    queryClient.invalidateQueries({ queryKey: ['incident-all-documents'] });
    showSuccess('Datos actualizados');
  };

  const clearFilters = () => {
    setFilters({ status: '', dateFrom: '', dateTo: '', category: '' });
    setSearchTerm('');
  };
          ...
          <div className="flex flex-col md:flex-row items-center gap-2 pr-2 w-full md:w-auto">
            ...
            <div className="relative group w-full md:w-80">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                <MagnifyingGlassIcon className="h-4 w-4 text-slate-400 group-focus-within:text-indigo-600 transition-colors" />
              </div>
              <input
                type="text"
                placeholder="Código, cliente, obra, categoría o folio SAP..."
                className="block w-full pl-12 pr-4 py-3.5 bg-slate-100/50 border border-transparent rounded-[1.5rem] text-sm font-bold text-slate-700 placeholder:text-slate-400 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all shadow-inner"
                value={searchTerm}
