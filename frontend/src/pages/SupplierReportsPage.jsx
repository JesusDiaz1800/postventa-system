  ...
  const { data: openIncidents, isLoading: incidentsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await api.get('/incidents/', {
        params: { status__in: 'nuevo,en_proceso,escalado_proveedor', page_size: 100 }
      });
      const allIncidents = response.data?.results || response.data || [];

      // Filter ONLY escalated to supplier
      const escalatedIncidents = allIncidents.filter(i => i.escalated_to_supplier || i.estado === 'escalado_proveedor');
          ...
          <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="inline-flex items-center px-4 py-2 bg-white/60 backdrop-blur-md border border-white/40 text-gray-700 font-medium rounded-xl hover:bg-white/80 hover:shadow-lg transition-all shadow-sm text-sm"
              >
                <ArrowDownTrayIcon className="h-5 w-5 mr-2 text-emerald-600" />
                Excel
              </button>
              <button
                ...
                className="inline-flex items-center px-4 py-2 bg-white/60 backdrop-blur-md border border-white/40 text-gray-700 font-medium rounded-xl hover:bg-white/80 hover:shadow-lg transition-all shadow-sm text-sm"
              >
                <DocumentArrowUpIcon className="h-5 w-5 mr-2 text-teal-600" />
                Adjuntar
              </button>
              <button
                onClick={() => setShowCreateModal(true)}
                className="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-teal-600 to-emerald-600 text-white font-bold rounded-xl shadow-lg shadow-teal-500/30 hover:shadow-teal-500/40 hover:-translate-y-0.5 transition-all text-sm"
              >
                ...
                <table className="min-w-full">
                  <thead>
                    <tr className="border-b border-gray-200/50 bg-gray-50/20">
                      <th className="px-6 py-5 text-left text-[11px] font-bold text-gray-400 uppercase tracking-widest">Reporte</th>
                      <th className="px-6 py-5 text-left text-[11px] font-bold text-gray-400 uppercase tracking-widest">Incidencia</th>
                      <th className="px-6 py-5 text-left text-[11px] font-bold text-gray-400 uppercase tracking-widest">Proveedor</th>
                      <th className="px-6 py-5 text-left text-[11px] font-bold text-gray-400 uppercase tracking-widest">Fecha</th>
                      <th className="px-6 py-5 text-center text-[11px] font-bold text-gray-400 uppercase tracking-widest">Estado</th>
                      <th className="px-6 py-5 text-right text-[11px] font-bold text-gray-400 uppercase tracking-widest">Acciones</th>
                    </tr>
                  </thead>
                    ...
                    {filteredReports.map((report) => (
                      <tr key={report.id} className="group hover:bg-white/60 transition-colors duration-150">
                        ...
                        <td className="px-6 py-4 text-center">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold border ${report.status === 'closed' ? 'bg-gray-100 text-gray-600 border-gray-200' :
                            ...
                            {report.status === 'closed' ? <CheckCircleIcon className="w-3.5 h-3.5 mr-1" /> :
                              report.status === 'enviado' ? <PaperAirplaneIcon className="w-3.5 h-3.5 mr-1" /> :
                                report.status === 'responded' ? <EnvelopeIcon className="w-3.5 h-3.5 mr-1" /> :
