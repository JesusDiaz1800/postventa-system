...
# Deployment script for Postventa System with SQL Server Express and Shared Folder
# Configuration: NB-JDIAZ25\SQLEXPRESS + \\192.168.1.161\Y:\postventa

set -e

echo "🚀 Starting deployment of Postventa System..."
echo "📁 Shared Folder: \\\\192.168.1.161\\Y:\\CONTROL DE CALIDAD\\postventa"
echo "🗄️  SQL Server: NB-JDIAZ25\\SQLEXPRESS"
echo "🗄️  Database: postventa_system"
echo ""
...
chmod +x scripts/setup-shared-folder-windows.py

# Setup shared folder structure
echo "📁 Setting up shared folder structure..."
python3 scripts/setup-shared-folder-windows.py

# Check if shared folder setup was successful
if [ $? -ne 0 ]; then
    echo "❌ Shared folder setup failed. Please check network access and permissions."
    exit 1
fi
...
docker-compose -f docker-compose-production.yml exec -T backend python manage.py migrate --settings=postventa_system.settings-sqlserver
docker-compose -f docker-compose-production.yml exec -T backend python manage.py collectstatic --noinput --settings=postventa_system.settings-sqlserver

# Create superuser
echo "👤 Creating superuser..."
docker-compose -f docker-compose-production.yml exec -T backend python manage.py shell --settings=postventa_system.settings-sqlserver << EOF
...
echo "   Frontend: http://192.168.1.161:3000"
echo "   Backend API: http://192.168.1.161:8000/api"
echo "   Admin Panel: http://192.168.1.161:8000/admin"
echo "   SQL Server: NB-JDIAZ25\\SQLEXPRESS"
echo ""
echo "👤 Default Login:"
echo "   Username: admin"
...
echo ""
echo "📁 Shared Folder:"
echo "   Path: \\\\192.168.1.161\\Y:\\CONTROL DE CALIDAD\\postventa"
